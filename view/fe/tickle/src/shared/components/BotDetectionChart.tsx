      ...
      <header className="border-b border-slate-200 p-5">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div className="min-w-0">
            <p className="text-[12px] font-bold text-red-600">Bot detection</p>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:min-w-[560px] xl:grid-cols-4">
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
              <p className="text-[11px] font-bold text-slate-500">오늘 탐지</p>
              <p className="mt-1 text-[24px] font-black leading-8 text-slate-950">
                {formatNumber(totalDetections)}
                <span className="ml-1 text-[12px] font-bold text-slate-500">건</span>
              </p>
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
              <p className="text-[11px] font-bold text-slate-500">차단 완료</p>
              <p className="mt-1 text-[24px] font-black leading-8 text-emerald-600">
                {formatNumber(totalBlockedBots)}
                <span className="ml-1 text-[12px] font-bold text-slate-500">건</span>
              </p>
              <p className="mt-1 text-[11px] font-black text-slate-500">정책 차단 기준</p>
            </div>

            <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
              <p className="text-[11px] font-bold text-slate-500">차단율</p>
              <p className="mt-1 text-[24px] font-black leading-8 text-blue-600">
                {formatPercent(blockRate)}
              </p>
              <p className={`mt-1 text-[11px] font-black ${blockRate >= targetBlockRate ? 'text-emerald-600' : 'text-red-500'}`}>
                목표 {formatPercent(targetBlockRate)}
              </p>
            </div>

            <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
              <p className="text-[11px] font-bold text-slate-500">피크 시간대</p>
