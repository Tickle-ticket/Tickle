...
export const PriceLegend = ({ prices, viewMode = 'grade' }: PriceLegendProps) => {
  ...
  return (
    <div className="absolute top-[80px] lg:top-4 left-4 z-20 group">
      <button className="flex items-center gap-1.5 sm:gap-2 bg-white/90 backdrop-blur-sm px-2.5 py-1.5 sm:px-4 sm:py-3 rounded-full shadow-lg text-xs sm:text-sm font-bold text-gray-800 border border-gray-200 hover:scale-105 transition-transform">
      ...
      <div className="absolute top-full left-0 mt-3 w-64 p-4 bg-white rounded-2xl shadow-xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform -translate-y-2 group-hover:translate-y-0">
        
        {!isCongestionMode && (
          <>
            <h4 className="text-sm font-extrabold text-gray-900 mb-3">등급 별 가격 안내</h4>
              ...
              {prices.map((item, idx) => (
                <div key={idx} className="flex justify-between items-center text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-gray-700">{item.priceGrade}석</span>
                  </div>
            ...
            <div className="border-t border-gray-100 pt-3">
              <h4 className="text-xs font-bold text-gray-500 mb-2">좌석 상태 안내</h4>
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2 text-sm">
                  <span className="block w-3 h-3 shrink-0 rounded-full bg-[var(--seat-disabled-top)]"></span>
                  <span className="font-medium text-gray-600">선택 불가 (예매 완료)</span>
                </div>
        ...
        {isCongestionMode && (
          <div>
            <h4 className="text-sm font-extrabold text-gray-900 mb-3">대기 혼잡도 안내</h4>
