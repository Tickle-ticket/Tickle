...
export const TimelineNav = ({ 
  ...
  if (isLoading) {
    return (
      <div className={`flex flex-col gap-8 ${className}`}>
        {[1, 2, 3].map((i) => (
          <div key={i} className={`bg-surface-active rounded animate-pulse ${size === 'small' ? 'w-20 h-4' : 'w-24 h-5'}`} />
        ))}
      </div>
    );
  }
  ...
  const colorStyles = {
    black: { activeText: 'text-black', activeDot: 'bg-black border-[#f8f8f8]', hoverText: 'hover:text-black', hoverDot: 'group-hover:bg-black' },
    blue: { activeText: 'text-primary', activeDot: 'bg-primary border-white', hoverText: 'hover:text-primary', hoverDot: 'group-hover:bg-primary' },
    primary: { activeText: 'text-primary', activeDot: 'bg-primary border-white', hoverText: 'hover:text-primary', hoverDot: 'group-hover:bg-primary' },
  };
  ...
  return (
    <div className={`relative flex flex-col ${currentSizes.gap} ${currentSizes.text} font-medium text-content-tertiary pl-2 ${className}`}>
      {/* 세로선을 모든 점들의 중앙을 완벽하게 관통하도록 배치 */}
      <div className={`absolute ${currentSizes.lineLeft} top-[10px] bottom-[10px] w-[1px] bg-black/10 z-0 ${lineStyle === 'dashed' ? 'border-l border-dashed border-black/20 bg-transparent' : ''}`}></div>
        ...
        >
          <div className={`${currentSizes.dotWrapper} flex justify-center items-center shrink-0`}>
            <span className={`rounded-full transition-all shadow-sm ${index === activeIndex
              ...
              : `${currentSizes.inactiveDot} bg-surface-active ${currentColors.hoverDot}`
              }`}></span>
          </div>
