...
export const TimelineNav = ({ 
  ...
  if (isLoading) {
    return (
      <div className={`flex flex-col gap-8 ${className}`}>
        {[1, 2, 3].map((i) => (
          <div key={i} className={`bg-gray-200 rounded animate-pulse ${size === 'small' ? 'w-20 h-4' : 'w-24 h-5'}`} />
        ))}
      </div>
    );
  }
  ...
  const colorStyles = {
    ...
    blue: { activeText: 'text-blue-600', activeDot: 'bg-blue-600 border-white', hoverText: 'hover:text-blue-600', hoverDot: 'group-hover:bg-blue-600' },
  ...
  return (
    <div className={`relative flex flex-col ${currentSizes.gap} ${currentSizes.text} font-medium text-zinc-500 pl-2 ${className}`}>
        ...
        >
          <div className={`${currentSizes.dotWrapper} flex justify-center items-center shrink-0`}>
            <span className={`rounded-full transition-all shadow-sm ${index === activeIndex
              ...
              : `${currentSizes.inactiveDot} bg-zinc-300 ${currentColors.hoverDot}`
              }`}></span>
          </div>
