...
export const Header = ({ className = "" }: HeaderProps) => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && isInitialSyncDone) {
      ...
      if (isMypageOpen) {
        if (
          url.searchParams.get("view") !== "mypage" ||
  ...
  }, [isMypageOpen, activeTab, isInitialSyncDone]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      if (inputValue.trim() && !isComposingRef.current) {
        closeMypage();
        closeDetail();
  ...
  return (
    <header
      className={`${selectedDetailId ? "hidden lg:flex" : "flex"} sticky top-0 z-50 bg-[#f8f8f8] -mx-6 px-6 md:-mx-10 md:px-10 items-center justify-between border-b border-black/10 pt-3 pb-3 mb-6 ${className}`}
    >
      {/* Left: Logo */}
