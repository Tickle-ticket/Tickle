...
export function UserAuthFrame({
  ...
  return (
    <div className="flex min-h-dvh w-full overflow-hidden bg-surface">
      <aside className="hidden shrink-0 flex-col justify-between border-r border-line-subtle bg-surface-subtle p-12 lg:flex lg:w-[42%] lg:min-w-[480px] xl:p-20">
        <div>
          ...
          <div className="mt-16 max-w-[420px]">
            <div className="mt-16">
              <h1 className="text-4xl leading-tight font-bold tracking-tight text-content">
        ...
        <p className="text-sm font-medium text-content-muted">© {new Date().getFullYear()} TICKLE. All rights reserved.</p>
      </aside>
      ...
      <div className="relative mx-auto flex min-h-dvh w-full max-w-6xl flex-col px-4 py-6 sm:px-6 lg:px-8">
        <main className={`mx-auto flex w-full flex-1 flex-col ${mainLayoutClass} ${contentWidth}`}>
          <Box variant="flat" padding="none" className="w-full bg-surface">
            <div className="w-full">
              <div className="mb-6 rounded-[20px] sm:rounded-[28px] border border-line bg-surface-muted/80 p-1 sm:p-1.5">
                  ...
                  {tabs.map((item) => (
                    <button
                      ...
                      className={`min-h-[44px] sm:min-h-[58px] rounded-[16px] sm:rounded-[22px] px-2 sm:px-4 text-[13px] sm:text-base font-bold transition ${
                        item.active
                          ? 'bg-surface text-slate-950 shadow-[0_10px_24px_rgba(15,23,42,0.08)]'
              ...
              {footer ? (
                <div className={compact ? 'mt-6 border-t border-line pt-4' : 'mt-8 border-t border-line pt-6'}>
                  {footer}
                </div>
              ) : null}
            </div>
