    ...
    <>
      ...
      <div className="bg-surface rounded-2xl border border-line shadow-sm">
        <div className="px-5 py-4 bg-surface-subtle border-b border-line rounded-t-2xl">
          <h3 className="font-extrabold text-[16px] text-content">예매자 정보</h3>
        </div>
        <div className="p-5 flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            <label className="sm:w-24 text-sm font-bold text-content-secondary shrink-0">예매자 이름</label>
            <input type="text" value={buyerName} onChange={e => setBuyerName(e.target.value)} placeholder="이름 입력"
              className="flex-1 px-4 py-3 rounded-xl border border-line bg-surface-subtle text-content text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all placeholder:text-content-muted" />
          </div>
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            <label className="sm:w-24 text-sm font-bold text-content-secondary shrink-0">이메일</label>
            <input type="email" value={buyerEmail} onChange={e => setBuyerEmail(e.target.value)} placeholder="이메일 입력"
              className="flex-1 px-4 py-3 rounded-xl border border-line bg-surface-subtle text-content text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all placeholder:text-content-muted" />
          </div>
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            <label className="sm:w-24 text-sm font-bold text-content-secondary shrink-0">전화번호</label>
      ...
      <div className="bg-surface rounded-2xl border border-line shadow-sm mb-4 lg:mb-0">
        <div className="px-5 py-4 bg-surface-subtle border-b border-line rounded-t-2xl">
          <h3 className="font-extrabold text-[16px] text-content">약관 동의</h3>
        </div>
        <div className="p-5 flex flex-col gap-3">
          {/* 전체 동의 */}
          <div className="flex items-center gap-3 px-4 py-4 bg-primary-subtle rounded-xl border border-primary-light">
            <Toggle checked={agreeAll} onChange={handleAgreeAll} size="small" />
            <span className="font-bold text-primary-hover text-sm">전체 동의합니다.</span>
          </div>

          {/* 예매 이용 약관 */}
          <div className={`rounded-xl border transition-colors ${agreeTerm1 ? 'border-primary-light' : 'border-line'}`}>
            <div className="flex items-center gap-3 px-4 py-3.5">
              ...
              <span className="flex-1 text-sm text-content-secondary">예매 이용 약관 (필수)</span>
              <button onClick={() => setTermExpand1(!termExpand1)} className="p-1 hover:bg-surface-muted rounded-full transition-colors">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
                  className={`transition-transform duration-300 text-content-muted ${termExpand1 ? 'rotate-180' : ''}`}>
            ...
            <div className={`grid transition-all duration-300 ease-in-out ${termExpand1 ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
              <div className="overflow-hidden min-h-0">
                <div className="px-4 pb-4">
                  <div className="bg-surface-subtle rounded-lg p-4 text-xs text-content-tertiary leading-relaxed max-h-32 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          {/* 개인정보 수집 동의 */}
          <div className={`rounded-xl border transition-colors ${agreeTerm2 ? 'border-primary-light' : 'border-line'}`}>
            <div className="flex items-center gap-3 px-4 py-3.5">
              ...
              <span className="flex-1 text-sm text-content-secondary">개인정보 수집 동의 (필수)</span>
              <button onClick={() => setTermExpand2(!termExpand2)} className="p-1 hover:bg-surface-muted rounded-full transition-colors">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
                  className={`transition-transform duration-300 text-content-muted ${termExpand2 ? 'rotate-180' : ''}`}>
                  <polyline points="6 9 12 15 18 9"></polyline>
                </svg>
              </button>
            </div>
