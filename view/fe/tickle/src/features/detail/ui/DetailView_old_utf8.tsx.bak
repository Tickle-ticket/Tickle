...
const navItems = [
  { id: 'info', title: '怨듭뿰 ?뺣낫' },
  { id: 'price', title: '媛寃??뺣낫' },
  { id: 'schedule', title: '怨듭뿰 ?쇱젙' },
  { id: 'details', title: '?곸꽭 ?뺣낫' },
];
          ...
          <div className="flex flex-col gap-8 pb-32">
            ...
            <div id="info" className="scroll-mt-32 w-full">
              <Box variant="flat" padding="medium" className="w-full border border-black/5">
                <div className="flex flex-col items-start gap-4">
                  <Title title="怨듭뿰 ?뺣낫" bottomBorder={true} className="!px-0 !pt-0 !pb-4 mb-1 w-full [&>div]:!px-0 [&_h1]:!text-xl" />
                  <div className="flex flex-col gap-6 w-full">
                    {[
                      { title: '?μ냼', descriptions: [data?.venue || '', data?.venueAddress || ''] },
                      { title: '怨듭??ы빆', descriptions: data?.notice?.split('\n') || [] }
                    ].map((item, idx) => (
            ...
            <div className="flex flex-col gap-8 w-full">
              ...
              <div id="price" className="scroll-mt-32 transition-all duration-500 ease-in-out w-full">
                <Box variant="flat" padding="large" className="w-full border border-black/5 flex flex-col gap-4 shadow-sm bg-white rounded-2xl">
                  <Title title="媛寃??뺣낫" bottomBorder={false} className="!px-0 !pt-0 !pb-2 mb-0 w-full [&>div]:!px-0 [&_h1]:!text-2xl shrink-0" />
                  <div className="w-full rounded-xl overflow-hidden border border-gray-100">
                    <Table
                      ...
                      data={data?.zonePrices?.map(p => ({ seat: p.grade, price: `${p.price.toLocaleString()}?? })) || []}
                      isLoading={isLoading}
                    />
                  </div>
                </Box>
              ...
              <div id="schedule" className="scroll-mt-32 transition-all duration-500 ease-in-out w-full">
                <Box variant="flat" padding="large" className="w-full border border-black/5 flex flex-col items-start shadow-sm bg-white rounded-2xl">
                  <div className="w-full flex flex-col items-start gap-4">
                    <Title title="怨듭뿰 ?쇱젙" bottomBorder={false} className="!px-0 !pt-0 !pb-2 mb-0 w-full [&>div]:!px-0 [&_h1]:!text-2xl" />
                      ...
                      <div className="w-full">
                        {selectedSchedule ? (
                          <div className="flex flex-col gap-4">
                            <Text typography="t5" fontWeight="bold" color="primary">?좏깮?섏떊 ?좎쭨???뚯감</Text>
                          ...
                          <div className="flex items-center justify-center h-32 border border-dashed border-gray-300 rounded-xl bg-gray-50/50">
                            <Text typography="t6" color="tertiary">愿?뚰븯???좎쭨瑜?罹섎┛?붿뿉???좏깮?댁＜?몄슂.</Text>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <div id="details" className="scroll-mt-32 w-full">
              <Box variant="flat" padding="medium" className="w-full border border-black/5">
                <div className="flex flex-col items-start gap-4">
                  <Title title="?곸꽭 ?뺣낫" bottomBorder={true} className="!px-0 !pt-0 !pb-4 mb-1 w-full [&>div]:!px-0 [&_h1]:!text-xl" />
                  {data?.detailImageUrl && (
