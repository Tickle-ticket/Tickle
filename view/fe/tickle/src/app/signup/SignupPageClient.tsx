      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {signupAccountTypes.map((accountType) => (
          <Link
            ...
            className="group flex min-h-[220px] flex-col justify-between rounded-[24px] border border-line bg-surface-subtle px-5 py-5 text-left shadow-[0_12px_30px_rgba(15,23,42,0.04)] transition hover:-translate-y-0.5 hover:border-primary-light hover:bg-surface hover:shadow-[0_18px_44px_rgba(15,23,42,0.08)] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-blue-500"
          >
            <div>
              <p className="text-sm font-bold text-primary">{accountType.label}</p>
              <h2 className="mt-5 text-xl font-black text-slate-950">{accountType.cardTitle}</h2>
              <p className="mt-3 text-sm font-medium leading-6 text-content-tertiary">{accountType.cardDescription}</p>
            </div>
            <span className="mt-8 inline-flex text-sm font-black text-primary">정보 입력하기</span>
          </Link>
        ))}
      </div>
    </UserAuthFrame>
