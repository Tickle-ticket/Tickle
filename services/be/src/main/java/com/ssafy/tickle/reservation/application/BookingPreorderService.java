...
public class BookingPreorderService {
    ...
    private static final Long start = 164571L;
     ...
     */
    @Transactional
    public BookingMockPreorderResponse mockPreorder(Long userId, BookingPreorderRequest request) {
        EventSession session = getSession(request.eventId(), request.sessionId());
        User user = getUser(userId);
        List<Long> seatIds = request.sessionSeatIds();
        ...
        BookingPreorderResponse preorder = BookingPreorderResponse.from(
                booking,
                tickets,
                getDiscountNameBySeatId(selectionBySeatId),
                holdExpiresAtOrNow(session.getId(), user.getId())
        );
        return BookingMockPreorderResponse.from(preorder, mockWinCount(seatIds));
    }

    private int mockWinCount(List<Long> seatIds) {
        return (int) seatIds.stream()
                .filter(MOCK_WIN_SESSION_SEAT_IDS::contains)
                .count();
    }
