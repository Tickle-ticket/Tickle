 ...
 * @param <T> 응답 데이터 타입
 */
// BeInternalClient가 BE 서버 응답을 이 타입으로 역직렬화한다. RestClient는 커스텀
// ObjectMapper 빈을 쓰지 않고 기본 컨버터를 만들며, 그 기본값은 모르는 필드를 만나면
// 예외를 던진다. 두 서비스의 응답 포맷이 한쪽만 먼저 바뀌어도 통신이 끊기지 않도록
// 모르는 필드는 무시한다.
