...
실행 (cwd = services/ai/):
    python -m macro.mouse_automation.recorder.human_recorder
...
def main():
    ...
    # services/ai/ 를 cwd 와 sys.path 에 보장 — 직접 실행 시도 대응
    ...
    parser.add_argument("--url", default=None, help="녹화할 웹사이트 URL (선택)")
    parser.add_argument("--target", default=None, help="타겟 설정에서 URL 가져오기")
    args = parser.parse_args()

    url = args.url
    if args.target and not url:
        from macro.mouse_automation.core._config import load_target
