...
브라우저 자동화 실험에서 수집하는 feature를 빠르게 확인하기 위한 문서입니다.


총 44개의 행동 데이터로 이루어져있고 사람과 매크로의 차이를 가를 수 있는 입력 데이터로 활용 가능한 지 분석합니다.


분석은 다음과 같은 순서로 진행했습니다.

- 수집을 위한 로컬 mock 서버 개발
- DOM 기반 매크로 개발
- 사람과 매크로 데이터 차이 분석 (SMD, 통계분석, 상관관계 분석, PCA) 
- 간단한 분류 모델 테스트 (Logistic Regression)
...
| `pre_click_path_300ms_total_distance_px` | 300ms 이동거리 | 클릭 직전 300ms 이동거리 | 이동 속도 확인 |
| `pre_click_path_300ms_straightness` | 300ms 직선성 | 클릭 직전 300ms 이동 직선성 | 1에 가까울수록 직선 |
| `pre_click_path_500ms_total_distance_px` | 500ms 이동거리 | 클릭 직전 500ms 이동거리 | 이동 속도 확인 |
| `pre_click_path_500ms_straightness` | 500ms 직선성 | 클릭 직전 500ms 이동 직선성 | 1에 가까울수록 직선 |
