...
ADR 문서를 Codex에서 직접 Notion에 작성할 수 있다.
...
4. **프로젝트 루트에서 Codex 실행** — `.Codex/settings.json`이 자동 로드되어 Notion MCP가 활성화됨
   ```bash
   Codex
   ```

5. **Notion 페이지 권한 추가** — ADR 페이지 및 API 명세서 페이지에서 `···` → `Connect to` → `Codex` Integration 선택
...
> `.Codex/settings.local.json`은 gitignore 처리되어 있으니 개인 설정은 자유롭게 사용 가능.
